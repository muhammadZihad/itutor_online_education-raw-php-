<?php 
    ...
    if (isset($_POST['comment_submit'])){
        if(!isset($_COOKIE['itutor_user'])){
            echo "<script type='text/javascript'>alert('You have to login to comment.');</script>";
        }
        else{
            ...
            $query = "update post_init set count_com = (select count(c_id) from comments where post_id = $post_id) where post_id = $post_id";
            mysqli_query($conn,$query);
        }
    }
